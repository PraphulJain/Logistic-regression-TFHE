...
void Sigmoid(LweSample* a, LweSample* array[][2], int n_bits, const TFheGateBootstrappingCloudKeySet* bk){
    LweSample* temp1 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
    LweSample* temp2 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
    LweSample* compare = new_gate_bootstrapping_ciphertext_array(2, bk->params);
    int i, k;
    for(k=0; k<n_bits; k++)
        bootsCONSTANT(&temp2[k], 0, bk);
    for(i=0; i<COUNT; i++){
        bootsCONSTANT(&compare[0], 0, bk);
        subtract(temp1, compare, a, array[i][0], n_bits,bk);
        multiplexer(temp2, array[i][1], temp2, compare, n_bits, bk);
    }
    for(k=0; k<n_bits; k++)
        bootsCOPY(&a[k], &temp2[k], bk);
}
...
//For calculating h(x)
void hypothesis(LweSample* result, LweSample** weight, LweSample** X, LweSample* array[][2], const int n_features, int n_bits, const TFheGateBootstrappingCloudKeySet* bk){
	int i, k;
	LweSample* temp1 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
	LweSample* temp2 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
	...
	for(i=0; i<n_features; i++){
		for(k=0; k<n_bits; k++)
            bootsMUX(&temp1[k], &X[i][0], &weight[i][k], &X[i][0], bk);
		Adder(temp2, temp1, result, n_bits, bk);
		for(k=0; k<n_bits; k++)
			bootsCOPY(&result[k], &temp2[k], bk);
	}
	//double_bits(&weight[n_features], n_bits, 2, bk);
	Adder(temp2, weight[n_features], result, n_bits, bk);//Last element of weights is the bias
	for(k=0; k<n_bits; k++)
		bootsCOPY(&result[k], &temp2[k], bk);
	Sigmoid(result, array, n_bits, bk);
}
...
void update_weight(LweSample** weight, LweSample** X, LweSample* y, LweSample* array[][2], const int n_features, int n_bits, const TFheGateBootstrappingCloudKeySet* bk){
	int i, j, k;
	LweSample* temp1 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
	LweSample* h = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
	LweSample* temp2 = new_gate_bootstrapping_ciphertext_array(2, bk->params);
    LweSample* temp22 = new_gate_bootstrapping_ciphertext_array(2, bk->params);
	LweSample* temp3 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
	LweSample* temp4 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
    LweSample* temp6 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
    LweSample* temp7 = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);
    LweSample* zeroes = new_gate_bootstrapping_ciphertext_array(n_bits, bk->params);

	for(k=0; k<n_bits; k++){
		bootsCONSTANT(&h[k], 0, bk);
        bootsCONSTANT(&zeroes[k], 0, bk);
    }
	hypothesis(h, weight, X, array, n_features, n_bits, bk);
    ...
    int base = 1;
    for(k=0; k<n_bits; k++){
        int ai = bootsSymDecrypt(&h[k], key)>0;
        int_answer += base*ai;
        base = base * 2;
        //int_answer |= (ai<<i);
    }
	...
	subtract(temp1, temp2, h, y, n_bits, bk);

	for(i=0; i<n_features; i++){
		//double_bits(&X[i], n_bits, 2, bk);
        for(k=0; k<n_bits; k++)
            bootsMUX(&temp3[k], &X[i][0], &temp1[k], &X[i][0], bk);

		bootsCONSTANT(&temp22[0], 0, bk);
		bootsCONSTANT(&temp22[1], 0, bk);

		//double_bits(&weight[i], n_bits, 4, bk);
        multiplexer(temp6, temp3, zeroes, temp2, n_bits, bk);
		subtract(temp4, temp22, weight[i], temp6, n_bits, bk);
        multiplexer(temp6, zeroes, temp3, temp2, n_bits, bk);
        Adder(temp7, temp4, temp6, n_bits, bk);
		for(k=0; k<n_bits; k++)
			bootsCOPY(&weight[i][k], &temp7[k], bk);
	}
	//Update weight[n_features]
	bootsCONSTANT(&temp22[0], 0, bk);
	bootsCONSTANT(&temp22[1], 0, bk);
    multiplexer(temp6, temp1, zeroes, temp2, n_bits, bk);
    subtract(temp4, temp22, weight[n_features], temp6, n_bits, bk);
    multiplexer(temp6, zeroes, temp1, temp2, n_bits, bk);
    Adder(temp7, temp4, temp6, n_bits, bk);
	for(k=0; k<n_bits; k++)
		bootsCOPY(&weight[n_features][k], &temp7[k], bk);
	//double_bits(&weight[n_features], 2*n_bits, 2, bk);
}

//For carrying out the whole logistic regression
void logistic_regression(LweSample** weight, LweSample* X[][2], LweSample** y, LweSample* array[][2], const int n_features, int n_bits, int n_epochs, const TFheGateBootstrappingCloudKeySet* bk){
	int epoch, tp, i, j, k;
	for(epoch=0; epoch<n_epochs; epoch++){
		printf("epoch %d\n", epoch+1);
		for(tp=0; tp<M; tp++){
            time_t start_time = clock();
			update_weight(weight, X[tp], y[tp], array, n_features, n_bits, bk);
            ...
            int base = 1;
            for(k=0; k<n_bits; k++){
                int ai = bootsSymDecrypt(&weight[0][k], key)>0;
                int ai1 = bootsSymDecrypt(&weight[1][k], key)>0;
                int ai2 = bootsSymDecrypt(&weight[2][k], key)>0;
                int_answer += base*ai;
            ...
            printf("updated weights = %d  %d  %d \n", int_answer, int_answer1, int_answer);
            printf("Time to execute this iteration %ld second\n",(end_time-start_time)/1000000);*/
            /////////////////////////////
		}
	}
}
...
int main(){
    ...
    logistic_regression(w, features, class, cypher, 2, BLEN, 1, bk);
    //Sigmoid(w[0], cypher, BLEN, bk);
    time_t end_time = clock();
    FILE* secret_key = fopen("secret.key","rb");
    TFheGateBootstrappingSecretKeySet* key = new_tfheGateBootstrappingSecretKeySet_fromFile(secret_key);
    fclose(secret_key);
